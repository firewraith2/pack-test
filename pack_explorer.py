...
class PackExplorer:
    ...
    def _build_ui(self):
        ...
        paned.add(controls, minsize=600)
    ...
    def _on_tree_select(self, event=None):
        ...
        # Preview WAN or PKDPX (detect inner type on-demand)
        if etype == "WAN":
            try:
                raw_data = self.manager.get_entry_data(idx)
                frames, info_text = generate_wan_frames(raw_data, self.manager, idx)
                if frames:
            ...
            try:
                ...
                # Check if inner type is WAN
                inner_type = detect_type(wan_data)
                if inner_type == "WAN":
                ...
                print(f"[ERROR] PKDPX preview failed: {e}")
                self.preview_panel.clear()
                self._last_previewed_idx = None
        else:
            self.preview_panel.clear()
    ...
    def _on_import_selected(self):
        ...
        if path:
            try:
                ...
                # Compress if checkbox is checked
    ...
    def _on_import_all(self):
        ...
        try:
            ...
            # Import files with optional compression
            ...
            for file_path in files:
                if file_path.is_file():
                    ...
                    self.manager.add_data(data, compress=compress)
                    count += 1
    ...
    def _on_export_all(self):
        ...
        try:
            dir_path = Path(directory)
            count = 0
            for idx, data in enumerate(self.manager.pack):
                etype, decompressed = detect_inner_type(data)

                # Use checkbox to decide on decompression
                export_data = data
                export_type = etype
                if etype.startswith("PKDPX(") and self.handle_compression.get():
            ...
            self._set_status(f"Exported {count} entries to {dir_path.name}/")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to export:\n{e}")
